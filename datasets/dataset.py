...
class InMemoryDigitsDataset(data.Dataset):
    """ Digits dataset."""
    def __init__(self, mode, text, img, total, transform=None, loader=default_loader, converter=None):
...
class InMemoryDigitsBatchTrain:
    def __init__(self, batch):
        transposed_data = list(zip(*batch))
...
def inmemory_train(batch):
    return InMemoryDigitsBatchTrain(batch)
...
def inmemory_dev(batch):
    return InMemoryDigitsBatchDev(batch)